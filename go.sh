...
# . ./go.sh camera_working # camera_working 從 nas 抓回本機 (only ds918, 家裡 LAN)
...
# 'camera_working' (縮寫 'cw') 方向和 0/1/2/3 相反，是從 nas 抓回本機。
...
#
# camera_working 單向同步，方向是 nas -> mac (和 0/1/2/3 相反)，只有 ds918，
# 只走家裡 LAN 的 rsync 873。
#
# 用途：camera_working/ 是給 windows 那台做 photoshop 用的工作區。windows 那邊
#       自己和 nas 同步 (go.bat)，mac 這邊只負責把做完的結果抓回來。
#
# 注意方向反了，危險的地方也跟著反：
#   1. --delete 砍的是「本機」。nas 上沒有的檔案，本機會被刪掉。
#      本機 camera_working/ 底下不要放任何只有本機有的東西，跑一次就沒了。
#   2. 所以「遠端」的 it_exists.txt 探測是關鍵的保護。nas 那個資料夾不見或是空的
#      話，--delete 會把本機清空。探測失敗就整個不做。
#   3. 沒有 -u。這是完全比照 nas，本機比較新的檔案也要被蓋掉。
if [ "$is_camera_working" -eq 1 ]; then

   # 本機 sentinel：確認 UltraFit256 有掛上。沒掛的話路徑還是會通，
   # 只是變成往內建硬碟倒 4.5G 進去。
   CHECKFILE="${dest_camera_working_dir_base}/it_exists.txt"
   if [ ! -f "$CHECKFILE" ]; then
      echo "no ${CHECKFILE} (UltraFit256 沒掛上?), stop"
   else
      echo "from : admin@192.168.123.163::${remote_918_camera_working_dir_base}"
      echo "to   : ${dest_camera_working_dir_base}   <-- --delete 砍的是這邊"
      ...
      if [ $? -eq 0 ]; then
         # @eaDir 是 Synology 自己產生的縮圖目錄，Thumbs.db / desktop.ini 是
         # windows 那台產生的，都不要拉到 mac 上
         echo "    ==== pull  163 -> local (--delete)"
         sshpass -p $pw rsync --port=873 -e "ssh -p 22" -a -v --delete --protocol=29 \
